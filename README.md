...
## Prompt Cache 对照
在同一 Qwen3.6上，对 PEVR 在线闭环 60 例先后开、关 llama.cpp `cache_prompt`。

| 指标 | 无缓存 | 有缓存 | 无/有 |
|---|---:|---:|---:|
| TTFT 中位数 | 4649 ms | 3233 ms | 1.44× |
| Prefill 中位数 | 4830 ms | 3349 ms | 1.44× |
| 缓存命中率 | 0.0% | 43.2% | — |
| 端到端 中位数 | 75.1 s | 68.5 s | 1.10× |

有缓存后 TTFT/prefill 大约缩短 44%，案例端到端只快约 9–10%，因为decoding仍占大头。
